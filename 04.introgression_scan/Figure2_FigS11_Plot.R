...
for(i in 1:nrow(intro_reg_collapsed)){
  ...
  if(i==2){
    ...
    #directory="<path_to_local_pool_seq_working_dir>/"  # not part of the Figshare deposit
...
i=9
  if(i==2){
    ...
    tmpBSAS <- df_spring %>% filter(Chr==chr & Pos >= s-padding_mb & Pos <= e + padding_mb)
    
    tmpBAAA <- df_autumn %>% filter(Chr==chr & Pos >= s-padding_mb & Pos <= e + padding_mb)
    ...
    tmpBSAS_plot <- ggplot()+     
      geom_rect(data=candidate_matrix,mapping=aes(xmin=start/1e6,xmax=end/1e6,ymin=0,ymax=1),
                fill="azure3",color=NA,alpha=1,inherit.aes = FALSE)+
      geom_point(data=tmpBSAS, aes(x=Pos/1e6, y=FST, color="black"), size=0.5)+
      scale_color_identity()+
      xlab(paste0("Chr ", c," position (Mb)"))+
      ylab(expression(paste(F[ST])))+
      xlim(s/1e6 - padding, 32.5)+
      ylim(0,1)+
      theme_classic()+
      theme(legend.position = "none")+
      theme(axis.title.x = element_text(size=6),
    ...
    tmpBAAA_plot <- ggplot()+     
      geom_rect(data=candidate_matrix,mapping=aes(xmin=start/1e6,xmax=end/1e6,ymin=0,ymax=1),
                fill="azure3",color=NA,alpha=1,inherit.aes = FALSE)+
      geom_point(data=tmpBAAA, aes(x=Pos/1e6, y=FST, color="black"), size=0.5)+
      scale_color_identity()+
      xlab(paste0("Chr ", c," position (Mb)"))+
      ylab(expression(paste(F[ST])))+
      scale_y_reverse(limits = c(1,0)) + 
      scale_x_continuous(position = "top", limits = c(s/1e6 - padding,32.5 ))+
      theme_classic()+
      theme(legend.position = "none")+
      theme(axis.title.x = element_blank(),
            axis.title.y = element_text(size=6),
...
directory <- "."  # set to wherever you want to save this figure locally

ggsave(plots1.6, file=paste0(directory,"combined_FST_introgression_scan_intro_reg_1_to_6.pdf"),
       units="mm",
       height=200,
       width=180)

ggsave(plots7.9, file=paste0(directory,"combined_FST_introgression_scan_intro_reg_7_to_9.pdf"),
       units="mm",
       height=200,
       width=180)

ggsave(plots1.6, file=paste0(directory,"combined_FST_introgression_scan_intro_reg_1_to_6.png"),
       units="mm",
       height=200,
       width=180)

ggsave(plots7.9, file=paste0(directory,"combined_FST_introgression_scan_intro_reg_7_to_9.png"),
       units="mm",
       height=200,
       width=180)
...
df_spring_filter <- df_spring %>% filter(FST >= 0)
df_spring_filter$ZFst <- scale(df_spring_filter$FST)
...
ggplot()+     
  #geom_rect(data=candidate_matrix,mapping=aes(xmin=start/1e6,xmax=end/1e6,ymin=0,ymax=20),
  #          fill="azure3",color=NA,alpha=1,inherit.aes = FALSE)+
  geom_point(data=tmpBSAS, aes(x=Pos/1e6, y=ZFst, color="black"), size=0.5)+
  scale_color_identity()+
  xlab(paste0("Chr ", c," position (Mb)"))+
  ylab(expression(paste(ZF[ST])))+
  xlim(s/1e6 - padding, e/1e6 + padding)+
  #ylim(0,20)+
  theme_classic()+
  theme(legend.position = "none")+
