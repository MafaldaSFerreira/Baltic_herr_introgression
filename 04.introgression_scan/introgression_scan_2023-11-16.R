...
##2026-06-12 
#updating filter for 30%: 12 haplotypes * 0.3 = 3.6 so cov > 3 (actualy >=4)
scan2_v01_baltic_alt_ref_summary_cov3 <- scan2_v01_baltic_alt_ref_summary$cov[scan2_v01_baltic_alt_ref_summary$cov[, "cov"] > 3,]
scan2_gr <- makeGRangesFromDataFrame(df=scan2_v01_baltic_alt_ref_summary_cov3, seqnames.field="group_name", keep.extra.columns=T)
scan2_width <- sum(data.frame(GenomicRanges::reduce(scan2_gr, min.gapwidth=50000))$width)
print(paste("NEW Scan 2 Percentage:", (scan2_width / 725670187) * 100, "%"))
#[1] "NEW Scan 2 Percentage: 0.022048583897522 %"
...
# Threshold 30% : 30 * 0.3 = 9. So cov > 8
scan3_v01_UK_alt_ref_summary_cov8 <- scan3_v01_UK_alt_ref_summary$cov[scan3_v01_UK_alt_ref_summary$cov[, "cov"] > 8,]
scan3_gr <- makeGRangesFromDataFrame(df=scan3_v01_UK_alt_ref_summary_cov8, seqnames.field="group_name", keep.extra.columns=T)
scan3_width <- sum(data.frame(GenomicRanges::reduce(scan3_gr, min.gapwidth=50000))$width)
print(paste("NEW Scan 3 Percentage:", (scan3_width / 725670187) * 100, "%"))
#[1] "NEW Scan 3 Percentage: 0 %"
...
# Change to 30% threshold: 48 haplotypes * 0.30 = 14.4 so cov > 13
scan4_v01_AtlAut_alt_ref_summary_cov13 <- scan4_v01_AtlAut_alt_ref_summary$cov[scan4_v01_AtlAut_alt_ref_summary$cov[, "cov"] > 13,]
scan4_gr <- makeGRangesFromDataFrame(df=scan4_v01_AtlAut_alt_ref_summary_cov13, seqnames.field="group_name", keep.extra.columns=T)
scan4_width <- sum(data.frame(GenomicRanges::reduce(scan4_gr, min.gapwidth=50000))$width)
print(paste("NEW Scan 4 Percentage:", (scan4_width / 725670187) * 100, "%"))
#[1] "NEW Scan 4 Percentage: 0 %"
...
# Change to 30% threshold: 46 haplotypes * 0.30 = 13.8 so cov > 13
scan7_v01_AtlSpring_summary_cov13 <- scan7_v01_AtlSpring_v_subarctic_alt_ref_summary$cov[scan7_v01_AtlSpring_v_subarctic_alt_ref_summary$cov[, "cov"] > 13,]
scan7_gr <- makeGRangesFromDataFrame(df=scan7_v01_AtlSpring_summary_cov13, seqnames.field="group_name", keep.extra.columns=T)
scan7_width <- sum(data.frame(GenomicRanges::reduce(scan7_gr, min.gapwidth=50000))$width)
print(paste("NEW Scan 7 Percentage:", (scan7_width / 725670187) * 100, "%"))
#[1] "NEW Scan 7 Percentage: 0.033072875846283 %"
...
# Change to 30% threshold: 6 haplotypes * 0.30 = 1.8 so cov > 1
scan10_v01_NorthSea_summary_cov1 <- scan10_v01_NorthSea_alt_ref_summary$cov[scan10_v01_NorthSea_alt_ref_summary$cov[, "cov"] > 1,]
scan10_gr <- makeGRangesFromDataFrame(df=scan10_v01_NorthSea_summary_cov1, seqnames.field="group_name", keep.extra.columns=T)
scan10_width <- sum(data.frame(GenomicRanges::reduce(scan10_gr, min.gapwidth=50000))$width)
print(paste("NEW Scan 10 Percentage:", (scan10_width / 725670187) * 100, "%"))
#[1] "NEW Scan 10 Percentage: 0 %"
...
summarize_intro <- function(intro_lists, target_list = "pac_red", plot_dir = "<path_to_local_output_dir>/", win_df = h125_20k_w, HapDistObj = herring_125, rec_co = 5){  # plot_dir not part of the Figshare deposit
