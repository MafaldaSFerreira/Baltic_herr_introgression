...
results_bs_as$results_simple <- paste0(sprintf("%.1f", -log10(results_bs_as$chi_Spring_top))  ," / ", sprintf("%.2f", results_bs_as$dAF_Spring))

results_ba_aa$results_simple <- paste0(sprintf("%.1f", -log10(results_ba_aa$chi_Autumn_top))  ," / ", sprintf("%.2f", results_ba_aa$dAF_Autumn))

View(cbind(results_bs_as$results_simple, results_ba_aa$results_simple))
...
# For the revisions, I will add the Fst for each SNP here. 
# Let's load the Fst file:
load("5.processing_pool_seq_data/fst_results/baltic_spring_vs_atlantic_spring_han_pops_Fst")
load("5.processing_pool_seq_data/fst_results/baltic_autumn_vs_atlantic_autumn_han_pops_Fst")


# 1. Prepare and Join for Spring
# We join results_bs_as with df_spring
# Mapping: intro_reg already contains the chromosome info, but we need 
# to match Position_Spring with df_spring$Pos and extract the Chr from intro_reg 
# OR use a more direct approach by parsing the intro_reg column.

results_bs_as_final <- results_bs_as %>%
  # Extract chromosome from the intro_reg string (everything before the first underscore)
  mutate(Chr = sub("_.*", "", intro_reg)) %>%
  left_join(df_spring, by = c("Chr" = "Chr", "Position_Spring" = "Pos")) %>%
  rename(Fst_Spring = FST) %>%
  # Move the new Fst column to a nice position
  relocate(Fst_Spring, .after = chi_Spring_top)

# 2. Prepare and Join for Autumn
results_ba_aa_final <- results_ba_aa %>%
  mutate(Chr = sub("_.*", "", intro_reg)) %>%
  left_join(df_autumn, by = c("Chr" = "Chr", "Position_Autumn" = "Pos")) %>%
  rename(Fst_Autumn = FST) %>%
  relocate(Fst_Autumn, .after = chi_Autumn_top)

# 3. Update the 'results' string column to include Fst (optional but helpful)
results_bs_as_final <- results_bs_as_final %>%
  mutate(results = paste0(results_simple, " / ", sprintf("%.2f", Fst_Spring)))

results_ba_aa_final <- results_ba_aa_final %>%
  mutate(results = paste0(results_simple, " / ", sprintf("%.2f", Fst_Autumn)))

# 4. View and Save
View(results_bs_as_final)
View(results_ba_aa_final)

write.table(results_bs_as_final, "5.processing_pool_seq_data/results/results_bs_as_with_fst.txt", sep="\t", quote=F, row.names=F)
write.table(results_ba_aa_final, "5.processing_pool_seq_data/results/results_ba_aa_with_fst.txt", sep="\t", quote=F, row.names=F)
